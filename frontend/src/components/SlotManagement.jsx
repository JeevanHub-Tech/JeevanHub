    ...
    const handleToggleDisable = async (slot) => {
        ...
        try {
            const res = await axios.put(
                `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/slots/template/${selectedDay}/${slot._id}`,
        ...
        const fetchSchedule = async () => {
            try {
                const res = await axios.get(
                    `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/getDoctorById/${doctorId}`,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
    ...
    const handleAddSlot = async () => {
        ...
        try {
            if (viewMode === 'exceptions') {
                ...
                const res = await axios.post(
                    `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/slots/template`,
                    payload,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
    ...
    const handleDeleteSlot = async (slotId) => {
        ...
        try {
            const res = await axios.delete(
                `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/slots/template/${selectedDay}/${slotId}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setAvailableSlots(res.data.availableSlots);
        } catch (error) {
    ...
    const handleUpdateSlot = async () => {
        ...
        try {
            if (viewMode === 'template') {
                const res = await axios.put(
                    `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/slots/template/${selectedDay}/${editSlotData._id}`,
                    editSlotData,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
                setAvailableSlots(res.data.availableSlots);
                                ...
                                return (
                                    <button 
                                        onClick={async () => {
                                            ...
                                            try {
                                                const res = await axios.put(
                                                    `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/slots/template/${selectedDay}/toggle-disable-all`,
                                                    { disable: !allDisabled },
                                                    { headers: { Authorization: `Bearer ${token}` } }
                                                );
                                                setAvailableSlots(res.data.availableSlots);
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    try {
                                        const res = await axios.post(
                                            `${BACKEND_URL || 'http://localhost:5000'}/api/doctors/slots/cancel-date`,
                                            { date: selectedExceptionDate },
                                            { headers: { Authorization: `Bearer ${token}` } }
                                        );
                                        setScheduleOverrides(res.data.scheduleOverrides);
