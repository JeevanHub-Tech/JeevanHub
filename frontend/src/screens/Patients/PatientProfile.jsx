...
const PatientProfile = () => {
    ...
    useEffect(() => {
        ...
        const fetchPatientData = async () => {
            try {
                const response = await axios.get(
                    `${BACKEND_URL || 'http://localhost:8080'}/api/patients/getPatient/${auth.user?.id}`,
        ...
        const fetchMedicalHistory = async () => {
            try {
                const response = await axios.get(
                    `${BACKEND_URL || 'http://localhost:8080'}/api/patients/${auth.user?.id}/medical-history`,
                    { headers: { Authorization: `Bearer ${auth.token}` } }
                );
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            const response = await axios.put(
                `${BACKEND_URL || 'http://localhost:8080'}/api/patients/updatePatient/${auth.user.id}`,
                payload,
                { headers: { Authorization: `Bearer ${auth.token}` } }
            );
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            const res = await axios.post(
                `${BACKEND_URL || 'http://localhost:8080'}/api/patients/${auth.user.id}/profile-image`,
                formData,
                {
                    headers: {
    ...
    const handleDeleteMedicalHistoryDoc = async (docId) => {
        ...
        try {
            const res = await axios.delete(
                `${BACKEND_URL || 'http://localhost:8080'}/api/patients/${auth.user.id}/medical-history/${docId}`,
                { headers: { Authorization: `Bearer ${auth.token}` } }
            );
            setMedicalHistory(res.data.medicalHistory || []);
        } catch (error) {
