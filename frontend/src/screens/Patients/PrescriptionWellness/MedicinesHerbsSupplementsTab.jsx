...
function MedicinesHerbsSupplementsTab() {
	...
	const rows = (bookings || []).flatMap((booking) =>
		(booking.recommendedSupplements || [])
			.filter((supp) => supp.published !== false)
			.map((supp) => ({ ...supp, doctorName: booking.doctorId?.name, addedAt: supp.addedAt }))
	);
				...
				{rows.map((row, i) => {
					...
					return (
						<Wrapper
							...
							{...wrapperProps}
							className={cn(
								"rounded-(--jh-radius-sm) border border-border p-4",
								isAvailable && "block transition-colors hover:border-primary hover:bg-primary/5",
							)}
						>
							<div className="mb-2 flex flex-wrap items-center justify-between gap-2">
								...
								<div className="flex items-center gap-2">
									{row.dosage ? <span className="text-sm text-muted-foreground">{row.dosage}</span> : null}
									{isAvailable ? <ChevronRight size={16} className="shrink-0 text-primary" /> : null}
								</div>
							</div>
							{row.instructions ? <p className="text-sm text-foreground/80">{row.instructions}</p> : null}
							<p className="mt-2 text-xs text-muted-foreground">
								{row.doctorName ? `Prescribed by Dr. ${row.doctorName}` : "Prescribed by your doctor"}
