...
const BulkMedicineUpload = () => {
    ...
    const autoSaveDraft = useCallback((currentRows, past, future) => {
        ...
        debounceTimeout.current = setTimeout(async () => {
            try {
                ...
                await axios.post(`${BACKEND_URL || 'http://localhost:8080'}/api/drafts/medicine`, {
                    present: currentRows
                }, {
    ...
    const handleClearDraft = () => {
        if (window.confirm("Are you sure you want to clear the active draft? Archived rows will be kept.")) {
            ...
            const token = localStorage.getItem('token');
            axios.post(`${BACKEND_URL || 'http://localhost:8080'}/api/drafts/medicine`, {
                present: newRows
            }, {
                headers: { Authorization: `Bearer ${token}` }
            }).then(() => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(`${BACKEND_URL || 'http://localhost:8080'}/api/medicines/add-bulk`, { medicines: validRows }, {
                headers: { Authorization: `Bearer ${token}` }
            });
