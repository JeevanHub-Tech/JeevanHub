        ...
        const fetchRetailerData = async () => {
            try {
                const response = await axios.get(
                    `${process.env.REACT_APP_AYURVEDA_BACKEND_URL || 'http://localhost:8080'}/api/retailers/getSingleRetailer/${auth.user?.id}`,
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            const response = await axios.put(
                `${process.env.REACT_APP_AYURVEDA_BACKEND_URL || 'http://localhost:8080'}/api/retailers/updateRetailer/${auth.user.id}`,
                payload,
                { headers: { Authorization: `Bearer ${auth.token}` } }
            );
